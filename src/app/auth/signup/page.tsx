...
export default function SignupPage() {
    ...
    const handleGoogleSignUp = async () => {
        ...
        try {
            ...
            await handleSignup(result.user);
        } catch (err: any) {
            console.error(err);
        ...
        <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4 relative font-sans">
            ...
            {isRedirecting && (
                <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-[#050505] animate-in fade-in duration-300">
                    ...
                    <h3 className="text-2xl font-black text-white uppercase tracking-tighter mb-2">Deploying Account</h3>
                    <div className="flex items-center gap-2 text-primary font-mono text-xs uppercase tracking-widest">
                        <Loader2 className="h-3 w-3 animate-spin" />
                        <span>Provisioning Admin Access...</span>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="bg-[#111] border border-white/10 p-8 rounded-3xl max-w-sm w-full relative animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="flex flex-col items-center text-center space-y-4">
                            ...
                            <h3 className="text-2xl font-black text-white uppercase tracking-tighter">System Locked</h3>
                            <p className="text-white/60 font-medium">Administrator quota reached. New registrations are currently suspended.</p>
            ...
            <div className="w-full max-w-md space-y-8">
                <div className="text-center">
                    ...
                    <h2 className="text-3xl font-black text-white uppercase tracking-tighter">
                        Admin <span className="text-primary italic">Registrar</span>
                    </h2>
                    <p className="mt-2 text-sm text-white/40 font-bold uppercase tracking-widest">Enrollment Protocol</p>
                </div>
                ...
                <div className="space-y-6 bg-white/5 p-8 rounded-3xl border border-white/5">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-white/40 uppercase tracking-widest mb-1.5 ml-1">Admin Email</label>
                            <input
                                type="email"
                                required
                                value={email}
                        ...
                        <div>
                            <label className="block text-xs font-bold text-white/40 uppercase tracking-widest mb-1.5 ml-1">Define Passkey</label>
                            <input
                                type="password"
                                required
                                value={password}
                        ...
                        >
                            {loading ? <Loader2 className="h-6 w-6 animate-spin" /> : 'Register Administrator'}
                        </button>
                    </form>
                    ...
                    <div className="relative py-2">
                        ...
                        <div className="relative flex justify-center text-[10px] uppercase font-bold tracking-[0.2em] text-white/20">
                            <span className="bg-[#050505] px-4">Fast Enrollment</span>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <span className="uppercase tracking-widest text-xs font-black">Link Google Identity</span>
                    </button>

                    <div className="text-center pt-2">
                        <Link href="/auth/login" className="group text-[10px] font-black text-white/40 hover:text-primary uppercase tracking-widest transition-all inline-flex items-center gap-2">
                            Existing Personnel Login <ChevronRight className="h-3 w-3 group-hover:translate-x-1 transition-transform" />
                        </Link>
                    </div>
                </div>
            </div>
