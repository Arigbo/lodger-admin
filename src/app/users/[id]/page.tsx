...
export default function UserProfilePage() {
            ...
            try {
                ...
                try {
                    const [propertiesSnap, leasesSnap, reportsSnap] = await Promise.all([
                        getDocs(query(collection(db, 'properties'), where('landlordId', '==', userId))),
                        getDocs(query(collection(db, 'leaseAgreements'), where('tenantId', '==', userId))),
                        getDocs(query(collection(db, 'userReports'), where('reportedUserId', '==', userId)))
                    ]);
    ...
    const handleBan = async () => {
        if (!user || !window.confirm(`Are you sure you want to ${user.banned ? 'unban' : 'ban'} this user?`)) return;
        try {
    ...
    if (error || !user) {
        return (
            <div className="p-10 rounded-[2rem] bg-rose-500/10 border border-rose-500/20 text-center max-w-2xl mx-auto mt-20">
                <p className="text-rose-500 font-bold text-xl mb-2">Error Loading User</p>
                <p className="text-white/60">{error || "User not found"}</p>
                <button onClick={() => router.push('/users')} className="mt-6 px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                    Back to Users
                </button>
            </div>
        );
    ...
    return (
        <div className="space-y-8 max-w-6xl">
            ...
            <div className="flex items-center gap-4">
                <button onClick={() => router.push('/users')} className="p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                ...
                <div>
                    ...
                    <p className="text-white/40 font-bold uppercase tracking-widest text-xs mt-2">Detailed account information & actions</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 p-10 rounded-[2.5rem] bg-white/5 border border-white/5 space-y-8">
                    <div className="flex items-start justify-between">
                        ...
                        <div className={cn(
                            ...
                            {user.role === 'landlord' ? <Home className="h-3 w-3 inline mr-2" /> : <GraduationCap className="h-3 w-3 inline mr-2" />}
                            {user.role}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="p-6 rounded-2xl bg-white/5">
                            <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-2">Joined</p>
                        ...
                        <div className="p-6 rounded-2xl bg-white/5">
                            <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-2">Status</p>
                        ...
                        <div className="p-6 rounded-2xl bg-white/5">
                            <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-2">User ID</p>
                            <p className="text-sm font-mono font-bold text-white/60">{user.id.slice(0, 12)}...</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/5">
                        <div className="text-center p-4 rounded-xl bg-white/5">
                            ...
                            <p className="text-xs font-bold text-white/40 uppercase tracking-widest mt-1">Properties</p>
                        </div>
                        <div className="text-center p-4 rounded-xl bg-white/5">
                            <FileText className="h-6 w-6 text-amber-400 mx-auto mb-2" />
                            <p className="text-2xl font-black">{userStats.leases}</p>
                            <p className="text-xs font-bold text-white/40 uppercase tracking-widest mt-1">Leases</p>
                        </div>
                        <div className="text-center p-4 rounded-xl bg-white/5">
                            <Shield className="h-6 w-6 text-rose-400 mx-auto mb-2" />
                            <p className="text-2xl font-black">{userStats.reports}</p>
                            <p className="text-xs font-bold text-white/40 uppercase tracking-widest mt-1">Reports</p>
                        </div>
                    </div>
                    ...
                    {showMessageBox && (
                        <div className="p-6 rounded-2xl bg-white/5 border border-primary/20 space-y-4">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="font-black uppercase tracking-tight">Send Message</h3>
                            </div>
                            <textarea
                                value={messageText}
                ...
                <div className="space-y-4">
                    <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/5 space-y-4">
                        <h3 className="text-xl font-black uppercase tracking-tighter mb-6">Admin <span className="text-primary italic">Actions</span></h3>
